...
(function() {
  ...
  function getQuestion(cleanStr, data, sel, depthTopic, prev, opts) {
    ...
    if (sel) {
      if (depthTopic) {
        question = chooseQuestion(['NONES', 'GENERIC_QS', 'X_QS', 'XY_QS', 'X_QS', 'XY_QS'], prev);
        X = sel;
        Y = depthTopic;
      } else {
      ...
      if (data.pos.nouns.length > 0) {
        if (data.pos.nouns.length > 1) {
          ...
          question = chooseQuestion(['NONES', 'GENERIC_QS', 'X_QS'], prev);
          X = chooseItems([...data.pos.nouns, ...data.pos.adjectives], depthTopic).X;
        }
      } else if (data.pos.adjectives.length > 0) {
        if (data.pos.verbs.length > 0) {
          question = chooseQuestion(['NONES', 'GENERIC_QS', 'X_QS', 'XY_QS', 'X_QS'], prev);
          items = chooseItems([...data.pos.verbs, ...data.pos.adjectives], depthTopic);
          X = items.X;
          Y = items.Y;
        } else {
          question = chooseQuestion(['NONES', 'GENERIC_QS', 'X_QS'], prev);
        ...
        question = chooseQuestion(['NONES', 'GENERIC_QS', 'NE_QS', 'NE_QS'], prev);
  ...
  function getReflection(tokens) {
    ...
    for (let word of tokens) {
      ...
      // handle 'us'
      else if (word === 'us' && language.coachPrepositions.indexOf(prev) >= 0) {
        word = 'you'; // we?
      } else {
  ...
  function tokenise(str, rsw) {
    if (!str) return [];
    const r = new RegExp(
      /(?:(?:\+?[01][\-\s.]*)?(?:[\(]?\d{3}[\-\s.\)]*)?\d{3}[\-\s.]*\d{4})|(?:[<>]?[:;=8>][\-o\*\']?[\)\]\(\[dDpPxX\/\:\}\{@\|\\]|[\)\]\(\[dDpPxX\/\:\}\{@\|\\][\-o\*\']?[:;=8<][<>]?|<3|\(?\(?\#?\(?\(?\#?[>\-\^\*\+o\~][\_\.\|oO\,][<\-\^\*\+o\~][\#\;]?\)?\)?)|(?:(?:http[s]?\:\/\/)?(?:[\w\_\-]+\.)+(?:com|net|gov|edu|info|org|ly|be|gl|co|gs|pr|me|cc|us|gd|nl|ws|am|im|fm|kr|to|jp|sg))|(?:http[s]?\:\/\/)|(?:\[[a-z_]+\])|(?:\/\w+\?(?:\;?\w+\=\w+)+)|<[^>]+>|(?:@[\w_]+)|(?:\#+[\w_]+[\w\'_\-]*[\w_]+)|(?:[a-z][a-z'\-_]+[a-z])|(?:[+\-]?\d+[,\/.:-]\d+[+\-]?)|(?:[\w_]+)|(?:\.(?:\s*\.){1,})|(?:\S)/,
      'gi'
    ); // eslint-disable-line
